...
- `renderer/src/` — React UI. **보편적 DB IDE 레이아웃**: 좌측 `ActivityRail`(아이콘) + `explorer`(섹션 패널) + `workspace`(에디터/결과) + 하단 `StatusBar`. `App.tsx`가 상태/오케스트레이션:
  ...
  - `SqlEditor` (`EditorTabs` 탭 스트립 + 툴바[연결 드롭다운·**포맷**·저장·실행 · LIMIT 없으면 "LIMIT 없음" 경고칩] + CodeMirror). CodeMirror 크롬은 `lib/cmTheme.ts`로 토큰화. ⌘↵ **현재 문장 실행** / ⌘S 저장 / ⇧⌘F 포맷.
    ...
    - **쿼리 파라미터**(`lib/queryParams.ts` + `components/ParamBar.tsx` + `lib/cmParams.ts`, #34): SQL의 `{{name}}`/`{{name:type}}`을 실행 대상 문장(`runTarget.text`)에서 스캔해 에디터↔결과 사이 **인라인 파라미터 바**를 상주시킨다. 값만 바꿔 ⌘↵ 반복 실행(모달 아님). 타입 `text`(기본, `'…'`+`''`이스케이프)·`number`(raw, 숫자 정규식 제약)·`raw`(따옴표 없이 그대로 삽입 — 식별자·동적 스키마/테이블명 보간용, 예 `laplacian_{{uid:raw}}`)·`date`(`DATE '…'`)·`multi`(`'a','b'` — 사용자가 `IN(…)` 작성)·**date-range**(`{{x.start}}`+`{{x.end}}` 접미 규약으로 한 위젯에 묶음, 한쪽만이면 일반 파라미터로 degrade). **치환은 `query:run` 전 순수 렌더러 문자열 변환**(서버 왕복·스키마 조회 0, main/ipc/client 무변경). 스캔은 `sqlStatements` 토크나이저와 같은 규칙으로 **문자열/식별자/주석 속 `{{…}}`를 무시**, `{{snippet:…}}`는 F26용 예약. 값은 **탭 세션 메모리 + 저장쿼리는 `localStorage`(`param:<savedQueryId>`)** 로 프리필. 필수값(빈값·비숫자·범위역전·빈 multi) 미충족 시 실행 차단 + 토스트. `실행될 SQL 보기`로 치환 결과 투명 노출. 컴파일된 SQL이 실행·history·(prod) 확인창에 쓰이고, 저장쿼리 본문은 템플릿 유지. 에디터의 `{{…}}`는 `.cm-param`(`--meta` 워시)로 강조.
