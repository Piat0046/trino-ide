...
- `renderer/src/` — React UI. **보편적 DB IDE 레이아웃**: 좌측 `ActivityRail`(아이콘) + `explorer`(섹션 패널) + `workspace`(에디터/결과) + 하단 `StatusBar`. `App.tsx`가 상태/오케스트레이션:
  ...
  - **메타데이터 조회(서버 SHOW, #52)**: MetadataPanel 3레벨의 "추가(＋)" 옆 **"조회" 버튼(`IconDownload`)** — 카탈로그(헤더)=`SHOW CATALOGS`, 카탈로그 행=`SHOW SCHEMAS FROM "cat"`, 스키마 행=`SHOW TABLES FROM "cat"."sch"`. `MetaProbeDialog`가 **명시 클릭 1회 = 서버 왕복 1회**로 목록을 조회(부제에 SQL+"서버 왕복 1회" 라벨), 검색+다중 체크박스로 골라 `upsertMetadata`(manual) **일괄 등록**. 이미 등록분은 "등록됨" 비활성(대소문자 무시 dedup), 브라우징-only 허용. **실행은 `query:run(recordHistory:false)` 재사용** — history 기록·자동 학습·메인 그리드를 오염시키지 않고 취소·errorInfo도 공짜(main/ipc/preload 무변경). SQL 조립·식별자 인용(`"`→`""`)·파싱은 `lib/showQueries.ts` 순수함수. **자동/폴링/host전환 시 조회 없음**(정체성: 사용자 안 누른 쿼리 금지 — 명시 옵트인만 허용). 드롭된 객체는 기존 `error-card`가 "Table/Schema does not exist"로 이미 노출.
