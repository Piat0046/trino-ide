...
- `renderer/src/` — React UI. **보편적 DB IDE 레이아웃**: 좌측 `ActivityRail`(아이콘) + `explorer`(섹션 패널) + `workspace`(에디터/결과) + 하단 `StatusBar`. `App.tsx`가 상태/오케스트레이션:
  ...
  - **Browser 탭(서버 드릴다운 탐색, #52)**: ActivityRail 5번째 섹션(`IconSitemap`, Metadata와 구분) `BrowserPanel`. host 셀렉터 + **카탈로그→스키마→테이블 드릴다운 트리** — 카탈로그 펼치면 `SHOW SCHEMAS FROM "cat"`, 스키마 펼치면 `SHOW TABLES FROM "cat"."sch"`(펼치기 = **명시 클릭 1회 = 서버 왕복 1회**). 최상위 카탈로그는 탭 열기·host 변경으론 조회 0 → **"카탈로그 불러오기 · 서버 조회 1회" 버튼**만 트리거. **비용 시그니처**: 미로드 확장노드 caret=앰버(`--warn`, "펼치면 서버 조회"), 로드(캐시)되면 중립 + hostbar 캡션. **캐시**: 조회 결과를 App `browseCache`(host별, 세션 한정)에 보관 → 섹션 전환·재펼침 시 서버 0(펼침 UI 상태만 리셋). 노드별/카탈로그 **새로고침**(명시 1회). 테이블 hover `＋` 등록 / 스키마 "전체" 일괄(로드된 미등록분, 추가 왕복 0) → `upsertMetadata`(manual) → **Metadata 탭·자동완성에 반영**, 이미 등록분 "등록됨"(대소문자 무시 dedup). **실행은 `query:run(recordHistory:false)` 재사용** — history·자동학습·메인 그리드 무오염(main/ipc/preload/shared 무변경). SQL 조립·식별자 인용(`"`→`""`)·파싱은 `lib/showQueries.ts` 순수함수. 인플라이트 SHOW는 host 전환 시 requestId 가드로 무효화. **자동/폴링 없음**(정체성: 사용자 안 누른 쿼리 금지). Metadata 탭 = 등록/학습본(순수 로컬), Browser 탭 = 서버 라이브 — 역할 분리. 드롭된 객체는 기존 `error-card`가 "does not exist"로 노출.
